...
def generate_data(args):
    ...
    index_list      = []     #对于给定的时刻t，它的index是：[t-history_seq_len, t, t+future_seq_len]
...
if __name__ == "__main__":
    ...
    future_seq_len  = 12                    # 未来数据长度，即F的大小

    train_ratio     = 0.6                   #训练集占总样本量的比例
    valid_ratio     = 0.2                   #验证集占总样本量的比例
    C               = [0]                   # selected channels  选择要使用的特征维度。例如在PEMS04中，我们只需要使用传感器采集的3中特征数据值的第一个维度，所以C=[0]
    steps_per_day   = 288                   # 5min

    name            = "PEMS04"
    dow             = True                  # if add day_of_week feature
    output_dir      = 'datasets/' + name
    data_file_path  = 'datasets/raw_data/{0}/{1}.npz'.format(name, name)    #原生数据位置
    graph_file_path = 'datasets/raw_data/{0}/adj_{1}.pkl'.format(name, name)   #图结构数据位置
